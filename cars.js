...
const ferraris = (ferrari) => ferrari.filter( e => e.brand.includes('Ferrari'));

// console.log(ferraris(cars));

// 2. Create a function nineties that returns all the cars with models between 1980 and 1990.

const nineties = (ninety) => ninety.filter( e => e.model>=1980 && e.model<=1990);
...
const newCarsList = (newList) =>  newList.map( e => `This ${e.brand} from ${e.model} costs $${e.price}`)

// console.log(newCarsList(cars));

// 4. Create a function bmwSum that returns the sum of all prices of the BMW cars.

const bmwSum = (bmw) => bmw.filter( e => e.brand.includes('BMW')).map( e => e.price).reduce( (acc, el) => acc+ el, 0);
